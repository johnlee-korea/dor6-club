...
## 변경 이력
- **v2.5.3** (2026-09-27): **구단운영 분석 판수 직접 입력** — 클럽원 불만 "100판 기준이라 스쿼드 바꾸기 전 선수가 섞인다". 범위 칩 옆 [최근 N판] 입력(5~300, 엔터·적용), 기기에 기억(`dor6.manage.range`), 저장된 경기보다 크면 모자란 만큼 자동 추가 조회, 기록이 부족하면 실제 경기 수 안내, 캡처 이미지에 "최근 N경기" 표기.
- **v2.5.2** (2026-09-27): **클럽 가입 문의 배너** — 일반 정보 전적 검색 결과 맨 위에 "도륙 클럽원 모집 중 · 가입 문의 →" 배너(카톡 오픈채팅 링크, `common.js` `DOR6.recruitUrl` 한 곳에서 변경). 캡처 이미지에는 들어가지 않음(사용자 결정). 목적: 캡처 이미지를 본 사람이 사이트에서 자기 닉을 검색할 때 클럽 간접 홍보.
- **v2.5.1** (2026-09-27): **[수정] 카카오톡 인앱 브라우저 이미지 저장** — 카톡 안에서는 blob 다운로드 링크가 막혀 "지원하지 않는 형식"으로 실패(사용자 제보, PNG 자체는 구조 검사 정상). UA에 KAKAOTALK이면 [이미지 저장] 대신 [🌐 크롬·사파리로 열기](`kakaotalk://web/openExternal?url=` 현재 주소·탭 유지) + 미리보기를 data URL로 바꿔 길게 눌러 저장 안내.
- **v2.5.0** (2026-09-27): **🎭 플레이스타일 세분화** — 슈팅 상세 지표 6개(감아차기·발리·땅볼·헤더 슈팅 비율, 측면 각도 슈팅, 후반 득점 비율; 유형 코드는 fc-info 골 유형과 대조해 확정) 추가(31지표), 조합 58 → 2조합 159 + **레어 3조합 22**(✨ 표시) + 단일 62. 판정: 레어 3조합 → 2조합 → 단일, 실력 지표(결정력·유효슈팅·득실·평점) 가중치 0.7. 공식 랭커 기준값 재생성. 실측: 클럽원 19명 서로 다른 스타일 11 → 18, 한 스타일 최다 6명 → 2명 / 랭커 150명 98가지·최다 4%.
